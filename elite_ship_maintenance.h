...
static inline bool ConsumeFuel(double fuelAmount, bool isLocalTravel) {
    ...
    if (g_state.PlayerShipPtr != NULL) {
        ...
        g_state.PlayerShipPtr->attributes.fuelLiters =
            fmax(g_state.PlayerShipPtr->attributes.fuelLiters - fuelLitersToConsume, 0.0);
...
static inline bool ShipRepair(int repairAmount, bool useCash) {
    ...
    int result = RepairHull(g_state.PlayerShipPtr, repairAmount, useCash ? costPerPoint : 0, 1);
