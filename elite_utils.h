...
static inline int my_rand(void)
{
	...
	{	// As supplied by D McDonnell	from SAS Insititute C
		r = (((((((((((lastrand_for_my_rand << 3) - lastrand_for_my_rand) << 3)
							+ lastrand_for_my_rand) << 1) + lastrand_for_my_rand) << 4)
					- lastrand_for_my_rand) << 1) - lastrand_for_my_rand) + 0xe60)
			& 0x7fffffff;
...
static inline bool string_begins_with(const char *prefixString, const char *fullString) // Made params const
{
	...
	{ 
        ...
        if (strlen(fullString) < l) return false;
...
{
	char *p;
    if (inputString == NULL) return NULL; // Handle NULL input
...
{
    if (inputString == NULL || firstWord == NULL) return; // Handle NULL input

	size_t l=strlen(inputString);
	size_t i=0,j=0;

	/* Strip leading spaces */
	while((i < l) && isspace((unsigned char)inputString[i])) i++; // Cast for isspace
