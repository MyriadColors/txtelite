...
static void display_game_status(char* location_buffer) {
    ...
    if (g_state.PlayerShipPtr != NULL) { // Calculate hull percentage
        ...
        (void)printf("\n\nLocation: %s | Cash: %.1f | Fuel: %.1fLY | Hull: %d%% | "
                     ...
                     location_buffer, ((double)g_state.Cash) / 10.0, ((double)g_state.Fuel) / 10.0, hull_percentage,
                     equipment_status, g_state.currentGameTimeSeconds);
    } else {
        printf("\n\nLocation: %s | Cash: %.1f | Fuel: %.1fLY | Time: %" PRIu64 " seconds > ", location_buffer,
               ((double)g_state.Cash) / 10.0, ((double)g_state.Fuel) / 10.0, g_state.currentGameTimeSeconds);
    }
}
